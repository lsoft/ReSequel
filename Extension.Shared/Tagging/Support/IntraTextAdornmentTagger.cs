...
namespace Extension.Tagging.Support
{
    /// <summary>
        ...
        where TAdornment : UIElement
    {
        ...
        private void AsyncUpdate()
        {
            ...
            if (this.snapshot != this._view.TextBuffer.CurrentSnapshot)
            {
                this.snapshot = this._view.TextBuffer.CurrentSnapshot;

                var translatedAdornmentCache = new Dictionary<SnapshotSpan, TAdornment>();

                foreach (var keyValuePair in this._adornmentCache)
                {
                    var translated = keyValuePair.Key.TranslateTo(this.snapshot, SpanTrackingMode.EdgeExclusive);
        ...
        /// Causes intra-text adornments to be updated synchronously.
        /// </summary>
        protected void RaiseTagsChanged(SnapshotSpanEventArgs span)
        {
            var handler = this.TagsChanged;
            if (handler != null)
                handler(this, span);
        ...
        private void HandleLayoutChanged(object sender, TextViewLayoutChangedEventArgs e)
        {
            ...
            {
                this._adornmentCache.Remove(span);
            }
        }
